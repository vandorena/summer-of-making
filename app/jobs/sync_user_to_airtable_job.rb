...
class SyncUserToAirtableJob < ApplicationJob
  ...
  def perform
    ...
    records = users_to_sync.map do |user|
      table.new({
        ...
        "hours" => user.user_hackatime_data&.total_seconds_across_all_projects&.fdiv(3600),
  ...
  def users_to_sync
    @users_to_sync ||= User.includes(:user_hackatime_data).order("synced_at ASC NULLS FIRST").limit(10)
  end
end
