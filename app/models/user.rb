...
class User < ApplicationRecord
  ...
  validates :display_name, presence: true, length: { maximum: 100 }, format: { with: /\A[^<>]*\z/, message: "nice try lmao" }
  validates :first_name, length: { maximum: 50 }, format: { with: /\A[^<>]*\z/, message: "nice try lmao" }, allow_blank: true
  validates :last_name, length: { maximum: 50 }, format: { with: /\A[^<>]*\z/, message: "nice try lmao" }, allow_blank: true
